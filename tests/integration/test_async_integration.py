...
class TestAsyncStandaloneIntegration:
    ...
    @pytest.mark.asyncio
    async def test_standalone_basic_operations(self, redis_container):
        ...
        client = await get_redis_client(f"redis://{host}:{port}", async_client=True)
    ...
    @pytest.mark.asyncio
    async def test_standalone_with_database_selection(self, redis_container):
        """Test Redis operations with database selection."""
        host = redis_container.get_container_host_ip()
        port = redis_container.get_exposed_port(6379)

        # Create clients for different databases
        client_db0 = await get_redis_client(f"redis://{host}:{port}/0", async_client=True)
        client_db1 = await get_redis_client(f"redis://{host}:{port}/1", async_client=True)
        ...
        assert await client_db0.get("db_key") == "db0_value"
        assert await client_db1.get("db_key") == "db1_value"

        # Verify cross-database isolation
        await client_db0.delete("db_key")
        assert await client_db0.get("db_key") is None
        assert await client_db1.get("db_key") == "db1_value"  # Still exists in db1

    @pytest.mark.asyncio
    async def test_standalone_multiple_operations(self, redis_container):
        ...
        assert await client.lrange("list_key", 0, -1) == ["item3", "item2", "item1"]
    ...
    @pytest.mark.asyncio
    async def test_standalone_connection_pooling(self, redis_container):
        ...
        for i, client in enumerate(clients):
            ...
            assert await client.get(f"pool_test_{i}") == f"value_{i}"

    @pytest.mark.asyncio
    async def test_standalone_error_handling(self, redis_container):
        ...
        with pytest.raises(
            ...
            await client.lpush("string_key", "item")  # Try to use list operation on string

        # Test operations on non-existent keys
        assert await client.get("non_existent_key") is None
        assert await client.exists("non_existent_key") == 0

    @pytest.mark.asyncio
    async def test_standalone_performance_basic(self, redis_container):
        ...
        for i in range(100):
            ...
            assert await client.get(f"perf_key_{i}") == f"value_{i}"
